// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.robot;

import edu.wpi.first.wpilibj2.command.Command;
import edu.wpi.first.wpilibj2.command.Commands;
import edu.wpi.first.wpilibj2.command.button.CommandXboxController;
import frc.robot.Constants.ArmConstants;
import frc.robot.Constants.OperatorConstants;
import frc.robot.Subsystems.Arm;
import frc.robot.Subsystems.Drive;
import frc.robot.Subsystems.Vision;
import io.github.oblarg.oblog.Logger;

public class RobotContainer {

  public CommandXboxController controller = new CommandXboxController(OperatorConstants.Driver_Controller_ID);
  Vision visionSub = new Vision();
  Drive driveSub = new Drive(visionSub);
  Arm armSub = new Arm();

  public RobotContainer() {
    Logger.configureLoggingAndConfig(this, false);
    driveSub.setDefaultCommand(driveSub.ArcadeDrive(controller::getLeftX, controller::getLeftY));
    armSub.setDefaultCommand(armSub.home().andThen(armSub.TrackGoal()));
    configureBindings();
  }

  private void configureBindings() {
    //shifts gears
    controller.a().onTrue(driveSub.Shift());
    controller.povUp().onTrue(armSub.TrackGoal(ArmConstants.upperGoal));
    controller.povRight().onTrue(armSub.TrackGoal(ArmConstants.middleGoal));
    controller.povDown().onTrue(armSub.TrackGoal(ArmConstants.stowed));
  }

  public Command getAutonomousCommand() {
    return Commands.print("hiii");
  }
}
