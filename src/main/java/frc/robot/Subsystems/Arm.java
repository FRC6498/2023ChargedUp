...
public class Arm extends SubsystemBase implements Loggable {
  ...
  public double highDropDistance = -122000;
  public double pickUpDistance = -125910;
  public double midDropDistance= -64500;
  public double slideMiddle;
  @Log
  public boolean slideCentered = true;
  //51 in
  ...
  public Arm() {
    ...
    slideMiddle = slideMotor.getSelectedSensorPosition();
    ...
    */
    ...
    armExtensionMotor.config_kP(0, 0.015);
    armExtensionMotor.config_kD(0, 0.05);
  ...
  public Command manualMoveSlide(DoubleSupplier percent) {
    return run(() -> {
      ...
      if (Math.abs(slideMotor.getSelectedSensorPosition() - slideMiddle) > 2048) {
        slideCentered = false;
      }
    });
  }
  ...
  public Command InitialArmCommand(DoubleSupplier leftTrigger, DoubleSupplier rightTrigger) {
    return //homeSlide().until(() -> slideHomeComplete == true)
        //.andThen(
    homeArm().andThen(
      moveArmPercent(-0.4).withTimeout(0.4),
      Commands.print("working"),
      retactArmPID(),    
  ...
  public Command homeSlide() {
    return run(()-> slideMotor.set(ControlMode.Position, slideMiddle))
      .until(()-> slideMotor.getSelectedSensorPosition() ==0).withTimeout(0.5)
    .andThen(
      runOnce(()-> slideCentered = true)
    );
  }
  /**
  ...
  public Command retractArm() {
    return runOnce(() -> armExtensionMotor.configClosedloopRamp(1)).andThen( 
    run(() -> armExtensionMotor.set(ControlMode.PercentOutput, 0.6))
        .until(() -> armExtensionMotor.isFwdLimitSwitchClosed() == 1))
        ;
  ...
  public Command extendArmHighPID() {
    return runOnce(() -> armExtensionMotor.configClosedloopRamp(0.5)).andThen( 
     run(() -> armExtensionMotor.set(ControlMode.Position,highDropDistance)))
    .withTimeout(1);
  } 
  /**
  ...
  public Command extendArmMidPID() {
    return runOnce(() -> armExtensionMotor.set(ControlMode.Position,midDropDistance))
    ;
  } 
  public Command retactArmPID() {
    return runOnce(()-> armExtensionMotor.set(ControlMode.Position, 0));
  }
  /**
